...
|---|---|---|---|---|
| Vinted | ⛔ `/api/v2/items/{itemId}` (GET) — **ENDPOINT MORT depuis le 2026-08-05** | GET, `credentials:include` | **404 avec un corps HTML pour TOUT article, vivant ou supprimé.** Cette ligne disait « 200 = présent, 404 = absent (test d'existence idempotent) » : c'est FAUX, et c'est ce qui a fait conclure des suppressions imaginaires jusqu'au 10/08. **Ne rend RIEN sur l'existence d'une annonce — ne jamais rebâtir de sonde de présence dessus.** Seul appelant restant : `lireDtoPublic` (repli de LIBELLÉS, dont l'échec ne produit qu'un `champs_manquants`) | vinted.js, `lireDtoPublic` |
| Vinted | `/api/v2/users/current` | GET | état de session (connecté ou non). **Vivant** — c'est lui qui tranche « session morte », jamais le code d'erreur du delete | vinted.js, `vintedSessionEtat` |
| Vinted | `/api/v2/items/{itemId}/delete` | **POST**, `X-CSRF-Token` (script inline) + `X-Anon-Id` (cookie `anon_id`) | **Vivant** (200 mesuré le 10/08 sur 8 suppressions réelles — à ne pas confondre avec le GET ci-dessus). 200/204 = supprimé ; **404 = déjà absent ⇒ succès idempotent** ; 401/403 ⇒ corps journalisé, session sondée, puis `needsUser` — **aucune suppression n'est jamais conclue d'un refus**. CSRF absent ⇒ requête NON envoyée et **aucune conclusion** : c'est le background qui lit l'état réel (`checkListingState`, `delete_confirmed_by='etat_annonce'`) | vinted.js, `deleteListing` |
