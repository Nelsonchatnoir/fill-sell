...
// Ne s'affiche que ce sur quoi l'utilisateur peut AGIR, en UNE phrase
// (consigne Nico 02/09 soir, deuxième passe — la première traduisait douze
// familles, c'était encore trop) : TROIS familles visibles, tout le reste
// est MUET — adresse Beebs, « Sans marque », champ Marque absent, couleur,
// matière, format/prix eBay, valeur hors listes, pré-rempli conservé,
// observabilité, sondes… : des détails que l'utilisateur ne peut ni
// comprendre ni corriger utilement, et qui l'inquiètent pour rien.
// Un warning d'une famille NON PRÉVUE n'affiche RIEN non plus — pas même
// une phrase générique. Le texte brut n'atteint jamais l'écran.
const WARN_FAMILLES_VISIBLES = [
  { re: /^description:.*(non remplie|non prise|refus|impossible|échec)/i,
    fr: "La description n'a pas pu être ajoutée — complète-la sur l'annonce en ligne.",
    en: "The description couldn't be added — complete it on the live listing." },
  { re: /obligatoire lu VIDE/i,
    fr: "Un champ obligatoire est resté vide — complète-le sur l'annonce en ligne.",
    en: 'A required field stayed empty — complete it on the live listing.' },
  { re: /^brouillon eBay auto-sauvegardé/i,
    fr: "eBay a gardé un brouillon en double — supprime-le dans Vendre > Brouillons.",
    en: 'eBay kept a duplicate draft — delete it under Selling > Drafts.' },
];
...
function warningsAffichables(job, lang = 'fr') {
  ...
  for (const w of job?.platform_fields?.warnings ?? []) {
    ...
    const fam = WARN_FAMILLES_VISIBLES.find((f) => f.re.test(msg));
    if (!fam) continue;
    if (!sorties.includes(fam[cle])) sorties.push(fam[cle]);
  }
  return sorties;
}
...
// ── ACTIVITÉ DU STOCK (2026-09-19) : DÉPÔTS **ET** REPUBLICATIONS ───────────
// Le bandeau du haut ne parlait que de republications : une publication lancée
// n'apparaissait NULLE PART (elle était bien chargée — jobsByInventaire porte
// tous les jobs — mais repubDernier jetait tout ce qui n'était pas
// action='republish'). Les trois briques ci-dessous servent LA file unique.
// AFFICHAGE PUR : rien ici n'écrit un job, un statut ni une étape.
//
// Clé d'une ligne = un article × une plateforme × une action. Deux jobs
// VIVANTS ne peuvent pas la partager (le serveur refuse le doublon), et
// l'entrée optimiste posée au clic porte la MÊME clé que la vraie ligne qui la
// remplace au poll suivant : le total ne double donc jamais à 20 s d'écart.
const cleActivite = (j) => `${j.inventaire_id}|${j.platform}|${j.action}`;
const ACTIVITE_TERMINAUX = ['published', 'sold', 'failed', 'cancelled', 'dry_run_completed'];
// Fini = le job ne bougera plus tout seul. 'recreated' est l'étape terminale
// d'une republication réussie (son statut, lui, peut rester 'processing').
const activiteFinie = (j) => ACTIVITE_TERMINAUX.includes(j.status)
  || (j.action === 'republish' && repubStepDe(j) === 'recreated');
// En cours = ce qui reste à faire SANS geste de l'utilisateur. Un 'needs_user'
// n'est PAS « en cours » : il a son encart d'alerte, séparé et exprès.
const activiteEnCours = (j) => !activiteFinie(j) && j.status !== 'needs_user'
...
const StockTab = memo(function StockTab({
  ...
  // Ouverture AUTOMATIQUE de la modale de conversion sur un plafond (journalise
  ...
  // 2026-09-04 : stock ILLIMITÉ pour tous (garde serveur levée). Un seul point
  // d'extinction ici plutôt qu'une condition ajoutée à chacun des quatre
  // affichages qui le lisent encore (bandeau « N articles restants », deux
  // encarts « limite atteinte », badge « Plan gratuit ») : à 0, aucun de leurs
  // seuils n'est franchi. Retour arrière avec STOCK_ILLIMITE = false.
  const quotaFree = STOCK_ILLIMITE ? 0 : compteArticlesQuota(items);
  // (La lecture de coin_config.free_stock_limit a été RETIRÉE le 2026-09-04
  // avec la ligne de quota : plus rien ici n'a de plafond à afficher, et
  // l'appel réseau n'avait plus de lecteur. La clé reste en base, intacte,
  // pour le retour arrière — ce composant ne la lit simplement plus.)
  ...
  // Fiche complète par id — la miniature et le titre du bandeau d'activité se
  ...
  // plus récent, ou une fenêtre de rafale ancrée sur le plus ancien job encore
  // en file). Chaque job terminé déplaçait donc l'ancre, et des jobs SORTAIENT
  // du lot en cours de route : c'est ce qui affichait « 0 sur 3 » puis
  // « 1 sur 2 » dix minutes plus tard (capture du 19/09 à 22:37 puis 22:47).
  // Un dénominateur qui bouge, c'est une progression qui ment.
  // Désormais : une clé entre dans la fournée quand elle commence à tourner,
  // elle en sort quand TOUTE la fournée est finie (ou si l'utilisateur l'a
  // arrêtée — et alors le compteur ET le bouton d'arrêt bougent ensemble,
  // puisqu'ils lisent le même ensemble au même instant).
  const activiteParCle = useMemo(() => {
    const m = new Map();
    for (const liste of Object.values(jobsByInventaire)) {
      for (const j of liste ?? []) {
        if (j.action !== 'publish' && j.action !== 'republish') continue;
        if (j.inventaire_id == null) continue;
        const c = cleActivite(j);
        const vu = m.get(c);
        if (!vu || Date.parse(j.created_at ?? 0) > Date.parse(vu.created_at ?? 0)) m.set(c, j);
      }
    }
    return m;
  }, [jobsByInventaire]);
  const [fourneeCles, setFourneeCles] = useState(null);
  useEffect(() => {
    const vivantes = new Set();
    for (const [c, j] of activiteParCle) if (activiteEnCours(j)) vivantes.add(c);
    setFourneeCles((prev) => {
      // Plus rien ne tourne : la fournée est close, le bandeau disparaît. Il ne
      // raconte pas la journée écoulée.
      if (!vivantes.size) return prev === null ? prev : null;
      const suivant = new Set(vivantes);
      // On GARDE les clés déjà comptées (terminées comprises) : c'est ce qui
      // fige le dénominateur. Une seule sortie anticipée : l'arrêt demandé par
      // l'utilisateur, qui ne compte nulle part (même doctrine que le bandeau
      // précédent) — et le bouton d'arrêt perd la même ligne au même rendu.
      if (prev) {
        for (const c of prev) {
          const j = activiteParCle.get(c);
          if (j && !estArretUtilisateur(j)) suivant.add(c);
        }
      }
      if (prev && prev.size === suivant.size && [...suivant].every((c) => prev.has(c))) return prev;
      return suivant;
    });
  }, [activiteParCle]);
  ...
  const activite = useMemo(() => {
    ...
    // serait le mensonge le plus visible de l'écran sur une file à l'arrêt.
    const hb = Date.parse(extensionStatus?.lastSeenAt ?? '');
    const hbMuet = !Number.isFinite(hb) || Date.now() - hb > 10 * 60 * 1000;
    return {
      lignes,
        ...
        {(activite||repubBandeau?.dryRuns>0||repubBandeau?.orpheline)&&(
          <div style={{background:"#fff",border:"1px solid #E7E3D8",borderRadius:20,padding:"12px 14px"}}>
            {activite&&(()=>{
              ...
              const titre=fiche?.title??activite.actif.title??(lang==='fr'?'Ton article':'Your item');
              // L'estimation se TAIT quand rien ne peut avancer : retenue
              // serveur (la file reprend demain — l'encart plafond le dit) ou
              // ordinateur muet depuis plus de 10 min. Annoncer « ~5-7 min »
              // sur une file à l'arrêt depuis des heures, c'est le mensonge le
                ...
                <button type="button" onClick={()=>setFileOuverte(true)} aria-haspopup="dialog"
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
                    {/* Miniature : la photo DÉJÀ en base, telle quelle. Pas de
                        ...
                        morte : le logo de la plateforme tient la place, jamais
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:13.5,fontWeight:700,color:"#10201B",letterSpacing:"-0.1px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                        {titre}
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:5,marginTop:3,fontSize:11.5,color:"#5C6560",minWidth:0}}>
                        <PlatformLogo platform={activite.actif.platform} size={13}/>
                        <span style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                          {LABEL_PF[activite.actif.platform]??activite.actif.platform}
                          <span style={{color:"#8A8578"}}> · {typeMot}</span>
                        </span>
                      </div>
                    </div>
                    {/* Le compteur d'abord, l'estimation APRÈS lui — jamais
                        l'inverse. Les deux lisent la fournée figée. */}
                    <div style={{flexShrink:0,textAlign:"right"}}>
                      <div style={{fontSize:13,fontWeight:700,color:"#10201B",fontVariantNumeric:"tabular-nums",whiteSpace:"nowrap"}}>
                        {lang==='fr'?`${activite.faites} sur ${activite.total}`:`${activite.faites} of ${activite.total}`}
                      </div>
                      {estimation&&(
        ...
        <div ref={listRef} className="stock-v2" style={{display:"flex",flexDirection:"column",gap:16,paddingBottom:16,...(isMobile&&stock.length===0?{order:1}:{})}}>
          ...
          <div style={{background:"#F6F5F1",borderRadius:16,padding:"6px 16px 16px",border:"1px solid #E7E3D8"}}>
            {/* ── L'EN-TÊTE TIENT SUR UNE LIGNE (2026-09-15 soir) ───────────
                ...
                et « Trier » — avec un flexWrap qui les faisait passer à deux ou
                trois lignes sur un téléphone, puis la pastille de compte. Sur
                l'iPhone de Nico, les articles commençaient aux deux tiers de
                l'écran. L'en-tête ne porte plus que ce qu'une section dit
                d'elle-même : son nom, son compte, sa valeur. Les filtres
                descendent sur leur propre ligne, juste en dessous.
            ...
            {/* ── Quota du plan : LIGNE SUPPRIMÉE le 2026-09-04 ────────────────
                ...
                a plus de quota d'articles à annoncer : ni le « Quota gratuit :
                N / 200 », ni sa version ambre à 80 %, ni le bouton
                « Quota gratuit atteint » qui ouvrait la modale de conversion
                sur stock_quota_atteint.
                « Stock illimité » n'est pas déplacé ici pour autant : ce
                n'était une information QUE parce qu'il y avait un plafond en
                face. Sans plafond, la phrase ne dit plus rien — une ligne de
                moins avant le premier article, dans la continuité de la
                densification du 04/09.
            ...
            {/* ⚠️ MODE OUVERT SEULEMENT (2026-09-15 soir) : au repos, c'est la
                ...
                ne connaît pas ce que TU as payé… ») et la sortie. La phrase se
                lit donc une fois, quand on ouvre, au lieu de tous les jours. */}
            {modePrixAchat&&(
            ...
            {/* ── É5.2 : republication en lot (bêta) — même patron que la
                ...
                échec post-clic). */}
            {/* ⚠️ MODE OUVERT SEULEMENT (2026-09-15 soir) : au repos, c'est la
                pastille « Republier N » qui arme le mode. Cet en-tête-ci reste
                le DÉTAIL, avec sa phrase (« Supprime puis recrée chaque
                annonce… N restantes ce mois-ci ») et sa sortie — lue à
                l'ouverture, plus tous les jours au-dessus du stock. */}
            {republishActif&&!modePrixAchat&&modeRepublish&&(
            ...
            {/* Fin du repli : ce qui suit (l'automatisation de la
                republication, É6) n'est PAS la liste du stock — le replier
                avec elle retirerait une fonction, ce que ce lot s'interdit. */}
            </>}
