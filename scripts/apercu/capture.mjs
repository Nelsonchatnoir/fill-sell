...
const PORT = 5199;
const URL = `http://localhost:${PORT}/scripts/apercu/lens-resultat.html`;
...
try {
  ...
  await page.screenshot({ path: path.join(SORTIE, 'lens-resultat-replie.png'), fullPage: true });
  console.log('écrit : screenshots-review/lens-resultat-replie.png');
  ...
  // Le libellé du bouton CHANGE une fois ouvert (« Détails » → « Le détail du
  // marché ») : on cible donc aria-expanded, pas le texte, et on reprend la
  // liste à chaque tour — sinon les index glissent sous les pieds.
  let n = 0;
  for (;;) {
    const ferme = page.locator('button[aria-expanded="false"]').first();
    if (!(await ferme.count())) break;
    await ferme.click();
    n += 1;
    if (n > 20) break;
  }
  await page.waitForTimeout(600);
  await page.screenshot({ path: path.join(SORTIE, 'lens-resultat-deplie.png'), fullPage: true });
  console.log(`écrit : screenshots-review/lens-resultat-deplie.png (${n} pliants ouverts)`);
