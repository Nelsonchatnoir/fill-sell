...
if (typeof chrome !== "undefined" && chrome.runtime?.onMessage) {
  ...
  // ── Relais de la sonde réseau (0.6.24) ─────────────────────────────────────
  // Même mécanique que vinted.js/ebay.js : la sonde (monde MAIN) postMessage
  // chaque capture ET chaque envoi ; on les relaie au background, qui survit à
  // la navigation. leboncoin.js n'avait AUCUN relais : après le Continuer
  // final, la page /options n'a plus la preuve du dépôt.
...
async function fillListingForm(job) {
  ...
  if (dryRun) {
    ...
    return { success: true, dryRun: true, warnings, unfilledRequired, discoveredRequired: enumerated, livraisonLbc: bilanLivraisonLbc };
  }
  ...
  const depotRequeteVue = async () => {
    try {
      ...
      return {
        ...
        // lbcDepotRequestSeen (background). detail = statut/details[]/id lus
  ...
  const refusApercu = async () => {
    ...
    // 0.6.24 : details[] de la réponse adsubmit = le texte EXACT de Leboncoin,
    // jamais un « n'a pas confirmé le dépôt » (cas fondateur : « Nous vous
    // invitons à supprimer toute mention d'un site internet autre que
    // leboncoin.fr… », 403 muet à l'écran).
    const detailsLbc = Array.isArray(sonde.adsubmit?.detail?.details) ? sonde.adsubmit.detail.details.filter((d) => d?.message) : [];
    if (sonde.seen && !sonde.illisible && Number(sonde.adsubmit?.status) >= 400 && detailsLbc.length) {
      const textes = detailsLbc.map((d) => d.message);
      const champs = detailsLbc.map((d) => d.field).filter(Boolean);
      console.warn(`[leboncoin] dépôt REFUSÉ par Leboncoin (adsubmit ${sonde.adsubmit.status}) : ${textes.join(" · ")}`);
      return {
        success: false, needsUser: true, warnings, unfilledRequired, discoveredRequired: enumerated,
  ...
  // cadence de re-clics de la 0.6.19 recliquait le Continuer final pendant
  // qu'il partait → deux annonces. Et la page /options servie ensuite, encore
  // vide, était lue « écran post-aperçu non reconnu » → reprise → un troisième
  // dépôt évité de justesse (job gelé à la main).
  // La sonde journalise désormais l'ENVOI (avant la réponse) : dès qu'un
  // adsubmit est parti, AUCUN re-clic, on attend sa RÉPONSE (≤ 60 s).
  //   · 2xx → dépôt ACCEPTÉ (id lu dans la réponse s'il y en a un) ;
  //   · ≥ 400 avec details[] → needs_user avec le texte EXACT de Leboncoin ;
  //   · aucune réponse en 60 s → « dépôt incertain », aucune reprise.
  // Seul un clic AVALÉ (aucun adsubmit parti) autorise encore un re-clic —
  // et la sonde est relue juste AVANT chaque re-clic.
  let depotAccepte = null;   // { status, id, statut, extrait } : adsubmit a répondu 2xx
  let refusAdsubmit = null;  // { status, details, extrait } : adsubmit a répondu ≥ 400
  const attendreReponseAdsubmit = async (budgetMs = 60_000) => {
    const fin = Date.now() + budgetMs;
    while (Date.now() < fin) {
      const sonde = await depotRequeteVue();
      const ad = sonde.adsubmit;
      if (ad && !ad.illisible && Number(ad.reponses) > 0 && Number.isFinite(Number(ad.status))) {
        return { sonde, status: Number(ad.status), detail: ad.detail ?? null };
      }
      // L'écran a avancé sans que la sonde ait lu la réponse (capture perdue,
      // sonde illisible) : la suite juge sur l'écran.
      if (findFreeCta() || /\/deposer-une-annonce\/(?:options|confirmation)/.test(location.pathname)) {
        return { sonde, status: null, detail: null, ecran: true };
      }
      await sleep(1000);
    }
    return { sonde: await depotRequeteVue(), status: null, detail: null, timeout: true };
  };
  const noterReponseAdsubmit = (r) => {
  ...
  // Refus adsubmit : le texte EXACT de Leboncoin. 403 sans details = DataDome.
  const verdictRefusAdsubmit = () => {
    const d = refusAdsubmit;
    const textes = (d.details ?? []).map((x) => x?.message).filter(Boolean);
    const champs = (d.details ?? []).map((x) => x?.field).filter(Boolean);
    if (textes.length) {
      return {
        success: false, needsUser: true, warnings, unfilledRequired, discoveredRequired: enumerated,
        lbcRefus: d,
    ...
    if (d.status === 403) {
      return {
        ...
        error:
          "CHALLENGE DATADOME : Leboncoin a refusé la requête de dépôt (403, vérification anti-robot sur l'API). " +
          "Ouvrir leboncoin.fr dans Chrome — sur CET ordinateur — et résoudre la vérification ; le job repartira au prochain passage." +
          ` — Observabilité: adsubmit HTTP 403 sans details ; ${dumpEcranVisible()}`,
      };
    }
    return {
      success: false, needsUser: true, warnings, unfilledRequired, discoveredRequired: enumerated,
  ...
  // (h1 « Boostez votre annonce ! » ou le CTA gratuit) jusqu'à 20 s. CTA → on
  ...
  const succesDepotAccepte = async (preuve, note) => {
    ...
    return {
      success: true, listingUrl: null, warnings, unfilledRequired, discoveredRequired: enumerated, livraisonLbc: bilanLivraisonLbc,
  ...
  for (let ecran = 0; ecran < 3 && !freeCta; ecran++) {
    ...
    while (!etape && estEncoreApercu() && reclicsApercu < 3
      ...
      const sonde = await depotRequeteVue();
      if (sonde.seen) {
        // adsubmit est parti (ou la sonde est illisible : prudence) : on n'y
      ...
      // Dernière lecture AVANT le clic : si adsubmit vient de partir, la boucle
      // reprend par la branche « attente de la réponse », jamais par un clic.
      if ((await depotRequeteVue()).seen) continue;
      reclicsApercu++;
      const note = `dépôt: Continuer final sans effet observable (aperçu inchangé, aucun adsubmit parti, aucun refus visible) — re-clic ${reclicsApercu}/3`;
      console.warn(`[leboncoin] ⚠️ ${note}`);
      warnings.push(note);
      realClick(encore);
    ...
    if (!etape) {
      // L'aperçu est parti (ou la boucle s'est arrêtée) sans écran reconnu :
      // adsubmit a-t-il parlé ?
  ...
  if (!preuve && (depotAccepte || surEcranOptions())) {
    return succesDepotAccepte(depotAccepte ? "adsubmit_2xx" : "ecran_options", "confirmation non lue après le chemin gratuit, mais le dépôt est accepté (adsubmit 2xx / écran /options) — annonce en vérification");
  }
  if (!preuve) {
    return {
      success: false, needsUser: true, depositUnconfirmed: true,
...
// Bilan du DERNIER réglage de livraison, joint au résultat du dépôt
// (platform_fields.livraison_lbc) — remis à null à chaque appel.
let bilanLivraisonLbc = null;

// ── RELEVÉ LIVE DU 24/09 (compte particulier, dépôt réel, rien soumis) ───────
// Le relevé du 20/09 disait vrai sur le CONTENU, faux sur la FORME — et
// c'est la forme que le code cherchait. 22 dépôts sur 22 (4 comptes, dont
// Louis 18/18) portaient « bloc « Activer la livraison » introuvable » :
//   · la case est un <button role="checkbox" aria-checked> dont le libellé
//     est dans aria-label (« Désactiver la livraison » quand elle est
//     cochée, « Activer… » sinon) ; le texte « Activer la livraison » est un
//     <span> VOISIN. lbcCliquableParTexte ne regardait que button / label /
//     a / [role=button] par leur TEXTE : il ne pouvait rien trouver ;
//   · le crayon des transporteurs : <button aria-label="Modifier les
//     transporteurs">, sans texte ;
//   · la fenêtre : [role=dialog] « Vos moyens de livraison », une ligne par
//     transporteur = <button role="checkbox" aria-checked>, le NOM est dans
//     le parent (pas dans le bouton), « Autres moyens de livraison » en
//     dernier, bouton « Valider » ;
//   · le format : crayon <button aria-label="Modifier"> dont le parent dit
//     « Colis petit — De 100 g à 250 g estimés » ; fenêtre « Choisissez un
//     format », trois <button role="radio"> de value S / M / L, bouton
//     « Continuer » → SECONDE fenêtre « Choisissez un poids » (onze paliers,
//     value en grammes : 100, 250, 500, 1000, 2000, 5000, 10000, 20000,
//     30000, 40000, 70000), l'estimation de Leboncoin pré-cochée, « Valider ».
//     Un compte PARTICULIER a donc bien un poids : c'est ce palier-là.
//   · « Fermer » (aria-label) referme une fenêtre SANS rien appliquer.
//
// 🚨 RIEN ICI N'EST BLOQUANT, JAMAIS : un dépôt ne rate pas pour un confort
...
//    poids sur le job → la page reste telle que Leboncoin l'estime.

const LBC_TRANSPORTEURS_PAGE = ["Courrier suivi", "Shop2Shop by Chronopost", "Mondial Relay", "Colissimo"];
const LBC_FORMAT_VALEUR = { petit: "S", moyen: "M", volumineux: "L" };
// ⚠️ MIROIR de src/utils/leboncoinColis.js (TRADUCTION + formatLbcDuJob) :
// format_colis fait foi, lbcFormatColis n'est qu'un repli des jobs d'avant.
const LBC_FORMAT_TRADUCTION = {
  "lettre": "Petit", "petit colis": "Petit", "moyen colis": "Moyen",
  "grand colis": "Volumineux", "très grand colis": "Volumineux", "tres grand colis": "Volumineux",
  "petit": "Petit", "moyen": "Moyen", "volumineux": "Volumineux",
};
function formatLbcDuJob(pf) {
  const tr = (v) => LBC_FORMAT_TRADUCTION[String(v ?? "").trim().toLowerCase()] ?? null;
  return tr(pf?.format_colis) ?? tr(pf?.lbcFormatColis);
}
const lbcTexteNu = (s) => String(s ?? "").normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/\s+/g, " ").trim().toLowerCase();

/** La fenêtre ouverte dont le titre matche (la plus récente). */
function lbcDialogue(motif) {
  const d = [...document.querySelectorAll('[role="dialog"], dialog')].filter((x) => motif.test(x.innerText || ""));
  return d.length ? d[d.length - 1] : null;
}
/** Le nom affiché à côté d'un contrôle : le premier ancêtre qui a du texte. */
function lbcLibelleVoisin(el) {
  let p = el;
  for (let i = 0; i < 4 && p; i++) {
...
function lbcCaseLivraison() {
  ...
  const span = [...document.querySelectorAll("span, label, p")].find((e) => /^activer la livraison$/i.test((e.innerText || "").trim()));
  return span?.parentElement?.querySelector('[role="checkbox"], input[type="checkbox"]') ?? null;
}
/** Le bloc « En livraison » tel qu'il s'affiche (pour relire ce qui est posé). */
...
async function poserLivraisonLbc(fields, warnings) {
  ...
  const format = formatLbcDuJob(fields);
  const poids = Number(fields?.lbcPoidsGrammes) > 0 ? Number(fields.lbcPoidsGrammes) : null;
  bilanLivraisonLbc = null;
  if (!(demandes && demandes.length) && !format && !poids) return; // rien demandé : on ne touche à rien
  ...
  const echec = (motif) => {
    ...
    warnings.push(`livraison : ${motif}`);
  };

  try {
    // 1. La livraison doit être ACTIVE, sinon les deux crayons sont inertes.
    const caseL = await waitFor(() => lbcCaseLivraison(), 6000);
    if (!caseL && !bilan.demandes) {
      // Compte PRO : pas de bloc « En livraison », le poids passe par son
      // propre champ « Poids du colis » (plus haut). Sans transporteurs
      // DEMANDÉS, il n'y a rien à dire : on ne fabrique pas d'alarme.
      return;
    }
    if (!caseL) {
      echec("la case « Activer la livraison » n'est pas sur la page — réglages d'envoi non posés (l'estimation de Leboncoin s'applique)");
    ...
    if (bilan.non_poses.length) {
      warnings.push(`livraison : ${bilan.non_poses.map((n) => `${n.nom} (${n.motif})`).join(", ")} — non coché`);
    }
    bilan.posee = !bilan.motif && !bilan.non_poses.length;
  } catch (e) {
