...
// port ouvert. tailleTrace jointe à TOUTES les issues, comme le canal classique.
...
function lancerRemplissageUnique(job) {
  ...
  etat.promesse = fillListingForm(job)
    .then(
      (result) => ({ ...result, tailleTrace: traceTaille() }),
      (err) => ({ success: false, error: String(err?.message ?? err), tailleTrace: traceTaille() }),
    )
...
// Forme comparable partagée (bloc ci-dessus) : apostrophes, guillemets, tirets
// et espaces insécables des libellés eBay (134 valeurs en ’, 1 029 en ", 4 439
// en ' dans ebay_item_aspects) ne font plus échouer un match.
...
async function urlToFile(url, index) {
  ...
  for (let essai = 0; essai <= DELAIS_MS.length; essai++) {
    ...
    if (res && res.ok) break;
    console.warn(
      `[ebay] photo ${index + 1} : lecture ${essai + 1}/${DELAIS_MS.length + 1} en échec ` +
