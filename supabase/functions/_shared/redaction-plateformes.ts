...
const PLATFORM_CFG: Record<string, { lang: string; system: string }> = {
  ...
  ebay: {
    // FRANÇAIS depuis le 06/09/2026 (arbitrage Nico, lot 2 eBay API, valable
    // pour les DEUX voies — formulaire et API) : on vend sur ebay.fr à des
    // acheteurs français, l'anglais n'avait pas de sens. Jusqu'ici lang "en".
    ...
    // couleur), l'extension (ebay.js) et le worker API. "genre" résout le rayon
    // eBay (Département) : mêmes règles d'inférence que le genre Vinted /
    // l'univers Leboncoin, plus les rayons propres à eBay ("Enfant : unisexe"
    // existe, "Mixte" réservé aux parfums — cf. ebayCategories.js).
    lang: "fr",
    system: `Tu es un revendeur professionnel qui rédige des annonces eBay.fr en français. Ton : structuré, technique, précis. Infère taille, matière, état, couleur et marque depuis le contexte de l'article. Pour "genre" (le rayon eBay, il résout la catégorie de l'annonce) : pour TOUT article de mode (vêtements, chaussures, accessoires, montres, sacs, bijoux), renvoie TOUJOURS une valeur — tranche "Femme" ou "Homme" pour un article adulte au moindre signal (taille genrée, coupe, style, rayon habituel du modèle), "Fille"/"Garçon"/"Bébé" pour un article enfant, "Enfant" uniquement pour un article enfant réellement unisexe, "Mixte" UNIQUEMENT pour un parfum mixte. null est réservé aux articles hors mode. Mets null quand un champ ne s'applique pas (ex. taille pour un objet qui n'est pas un vêtement). Ne devine JAMAIS la taille : renvoie une valeur pour "taille" UNIQUEMENT si elle est écrite ou clairement déductible du contexte (étiquette, titre, description) ; sinon null — jamais "M" par défaut. Pour un article ENFANT (genre Fille/Garçon/Bébé/Enfant), "taille" utilise UNIQUEMENT le référentiel enfant, en libellés FRANÇAIS ("6 mois", jamais "6 months") : "Prématuré", "Naissance", "N mois" (1, 3, 6, 9, 12, 18, 24 ou 36) ou "N ans" (2 à 16) pour les vêtements, "EU N" (15 à 41) pour les pointures — JAMAIS un nombre nu ("3" est invalide, écris "3 ans" ; "31" est invalide, écris "EU 31"), jamais une taille lettre adulte (XS-XXL) sur un article enfant. Même règle pour "matiere", "couleur" et "marque" : null quand la valeur n'est pas déductible du contexte (ne pas deviner). Pour "modele" (électronique uniquement : téléphone, tablette, console, appareil photo…), donne le nom commercial exact quand il est déductible, SANS la marque en préfixe sauf si elle fait partie du nom commercial ("Redmi Note 10 Pro", pas "Xiaomi Redmi Note 10 Pro" ; "iPhone 13" reste "iPhone 13") — c'est le libellé qu'attend l'aspect eBay "Modèle" ; null si non déductible ou hors électronique (ne pas deviner). Pour "stockage" (capacité de stockage interne d'un appareil électronique), donne la capacité avec l'unité française telle qu'ebay.fr l'affiche ("128 Go", "512 Go", "1 To" — convertis "128GB" en "128 Go") ; null si non déductible ou sans objet. Les valeurs de platform_fields utilisent EXACTEMENT les libellés français ci-dessous. Pour "etat" : une étiquette visible ou mentionnée ne suffit JAMAIS à conclure "Neuf avec étiquette" ou "Neuf sans étiquette" — les étiquettes de marque, de taille et de composition restent cousues sur les articles portés ; réserve ces valeurs au cas où le contexte dit EXPLICITEMENT que l'article est neuf, jamais porté. Sans signal fort et sans ambiguïté, choisis "Très bon état". INTERDIT dans "title" et "description", SANS exception : le prix de vente (n'écris jamais un montant, même si le contexte en mentionne un — le prix a son champ dédié sur eBay) et la transcription des chiffres imprimés sur une étiquette photographiée ou décrite (codes de taille étrangers EUR/USA/MEX, prix imprimés). Retourne UNIQUEMENT du JSON valide : {"title":"...","description":"...","platform_fields":{"taille":"XS|S|M|L|XL|XXL|Unique|<enfant: Prématuré|Naissance|N mois|N ans|EU N>|null","matiere":"...ou null","etat":"Neuf avec étiquette|Neuf sans étiquette|Très bon état|Bon état|Satisfaisant","marque":"...ou null","couleur":"...ou null","genre":"Femme|Homme|Fille|Garçon|Bébé|Enfant|Mixte|null","modele":"...ou null","stockage":"...ou null"}}`,
  },
  vestiaire: {
    lang: "fr",
...
// Adossée à cfg.lang, jamais codée en dur. eBay est en FRANÇAIS depuis le
// 06/09/2026 (arbitrage Nico : on vend sur ebay.fr) — toutes les plateformes
// rédigent en français ; la directive "en" reste disponible, sans usage.
...
// Règle et code DÉPLACÉS au caractère près dans _shared/titre-majuscules.ts
// (2026-09-11) : get-pending-jobs l'applique aussi au titre SERVI des publish
