...
-- Le pipeline GPT Image de generate-listing retombe photo par photo sur
-- l'original en cas d'échec (fetch raté, API en erreur, upload raté). Une
-- option ia_light/ia_advanced pouvait donc facturer 9 ou 32 pour des photos
-- jamais retouchées. Règle : AVANT de facturer la part photos, vérifier que
-- les photos du job portent RÉELLEMENT une retouche ; sinon part photos = 0
-- (photos_billed:false dans la réponse — le front affiche le message).
--
-- Détection d'une photo retouchée — même frontière que le client (StockTab) :
--   · entrée objet avec url sous /enhanced/ (chemin storage canonique du
--     pipeline : ${user}/enhanced/…). ⚠️ le champ `type` de la photo 0 vaut
--     'original' MÊME retouchée (relevé du code) — l'URL fait foi, le type
--     n'est qu'un indice complémentaire ;
--   · type commençant par 'enhanced' (photos 1..n du pipeline actuel) ;
--   · clés enhanced/bg_removed non vides (schéma historique) ;
--   · string nue sous /enhanced/ (photos réutilisées, aplaties par le flux
--     « déjà retouchées » du stepper).
--
-- Seul ajout par rapport à 20260805000000 (v5 réservation/capture) : le bloc
-- « retouche livrée ? » + photos_billed dans réponse et ledger. Idempotent.
