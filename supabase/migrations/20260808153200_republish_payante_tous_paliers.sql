-- Republication : PAYANTE POUR TOUS LES PALIERS (2026-08-08, étape 2).
--
-- 1. La gratuité plan meurt : le bloc « v_inclus → prix 0 » (premium/pro/
--    comped) est retiré, price_republish (1) se lit en config pour tout le
--    monde. v_inclus ne survit que dans le label télémétrie usage_logs.plan
--    (information de plan, pas un prix).
-- 2. L'auto-republication (source='auto', toujours réservée aux Pro — un
--    avantage de FONCTIONNALITÉ, pas de prix) devient donc un débit
--    quotidien. Pour qu'un échec ne soit JAMAIS silencieux : quand l'auto
--    échoue faute de Pépites, la fonction pose
--    platform_settings.vinted.republish_auto.derniere_erreur =
--    'pepites_insuffisantes' (+ derniere_erreur_le), que le bloc de réglage
--    de StockTab affiche en clair ; le marqueur est effacé au premier succès
--    auto suivant. Rien à changer côté extension : c'est le RPC qui écrit.
-- Tout le reste (gardes extension/fraîcheur/version, plafond auto, doublon,
-- cadence 24 h, job, ledger) est copié à l'identique de la définition en
-- prod relevée avant cette migration.

create or replace function public.spend_coins_and_republish(
  p_inventaire_id bigint, p_vinted_item_id text,
  p_source text default 'manuel', p_prix_republication numeric default null
)
returns jsonb
language plpgsql
security definer
