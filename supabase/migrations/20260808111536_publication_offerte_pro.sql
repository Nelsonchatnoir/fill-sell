-- Publication OFFERTE en Pro (2026-08-08, demande Nico — promesse déjà faite
-- aux clients : « En pro… publication et republication gratuite »).
--
-- La part PUBLICATION (price_per_platform × nb plateformes) passe à 0 pour
-- les profils is_pro = true. Ne changent PAS :
--   · la génération d'annonce IA (price_generate, débitée à part au step 1) ;
--   · la retouche photos (0/9/32, option explicite, une fois par article) ;
--   · le mécanisme de réservation/capture — une réservation à 0 traverse
--     settle/expire/refund sans effet (aucune division par unit_price,
--     vérifié sur les trois fonctions de règlement avant cette migration) ;
--   · la republication (spend_coins_and_republish), déjà gratuite en
--     premium/pro.
-- is_pro est la SEULE condition (pas is_premium, pas is_comped : un comped
-- est un Premium offert — la gratuité de publication est un droit PRO).
--
-- Le client (ListingPreviewScreen) affiche 0/« offerte » pour les Pro mais ne
-- décide rien : cette fonction reste la seule autorité de débit.
...
BEGIN
  ...
  SELECT extension_last_seen_at, lang, COALESCE(is_pro, false)
    INTO v_ext_seen, v_lang, v_is_pro
  ...
  -- ── Publication OFFERTE en Pro (2026-08-08) ───────────────────────────────
  -- 0 Pépite/plateforme pour is_pro. Génération et retouche photos restent
  -- des postes distincts, non couverts par cette gratuité.
  IF v_is_pro THEN
    v_price_unit := 0;
  END IF;
