-- ═══════════════════════════════════════════════════════════════════════════
-- FENÊTRE DE FRAÎCHEUR DE L'EXTENSION — 7 JOURS
-- ⛔ NON APPLIQUÉE — à montrer à Nico avant exécution (2026-08-04).
-- ═══════════════════════════════════════════════════════════════════════════
-- Les deux RPC payantes testaient `extension_last_seen_at IS NULL`, c'est-à-dire
-- « une extension a-t-elle été vue AU MOINS UNE FOIS », jamais « est-elle
-- encore là ». Un compte qui a installé puis désinstallé passe la garde, est
-- DÉBITÉ, et son job reste 'pending' sans personne pour le ramasser : c'est
-- exactement l'incident des 23 jobs.
--
-- Seuil : 7 JOURS (arbitrage Nico). Pas un seuil en minutes : la file est
-- asynchrone PAR DESIGN — commander une publication depuis le mobile avec le
-- PC éteint est un usage légitime, et un seuil court le tuerait. 7 jours
-- attrape la désinstallation sans jamais refuser une nuit, un week-end ou
-- quelques jours sans ordinateur (l'extension polle toutes les 2 minutes dès
-- que Chrome est ouvert : un seul lancement dans la semaine suffit).
--
-- Nouveau reason 'extension_stale', DISTINCT de 'extension_required' : le
-- premier veut dire « réveille-la », le second « installe-la ». Les fronts
-- affichent déjà le champ `message` renvoyé par la RPC — aucun déploiement
-- front n'est nécessaire, y compris pour les fronts figés.
--
-- ⚠️ Les deux définitions ci-dessous sont RECOPIÉES DEPUIS LA PROD
-- (pg_get_functiondef), pas réécrites de mémoire — même méthode que la
-- migration 20260728100000. Le SEUL ajout est le bloc « Fenêtre de fraîcheur ».
