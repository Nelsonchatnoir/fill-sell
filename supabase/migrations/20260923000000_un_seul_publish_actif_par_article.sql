-- ════════════════════════════════════════════════════════════════════════════
-- UN SEUL JOB DE PUBLICATION ACTIF PAR ARTICLE ET PAR PLATEFORME
-- (feu vert Nico, 2026-09-23)
-- ════════════════════════════════════════════════════════════════════════════
-- Bonnet Jack & Jones de lesnesarthur : DEUX jobs de publication Vinted actifs
-- pour le même article, créés le 18/09 à 17:24 et le 19/09 à 13:05. Rien ne
-- l'empêchait — la garde `already_published` de spend_coins_and_publish ne
-- regardait QUE les statuts 'pending' et 'processing'. Le premier job était en
-- 'needs_user' : il ne comptait pas, et le second est passé.
--
-- Deux verrous, l'un derrière l'autre :
--   1. l'INDEX (posé hors migration le 23/09, repris ici en IF NOT EXISTS pour
--      que le schéma soit reconstructible) — le dernier mot, quel que soit le
--      chemin de création, présent ou futur ;
--   2. la RPC, qui refuse AVANT tout débit et rend une raison que l'app sait
--      déjà afficher (« Déjà en ligne (ou en file) sur : … »).
--
-- ⛔ PÉRIMÈTRE : action='publish' UNIQUEMENT. `republish` et `delete` ont leurs
--    propres machines à états et peuvent légitimement coexister.
-- ⛔ Le parc a été dédoublonné AVANT (3 groupes, 5 jobs passés en 'cancelled'
--    avec un motif neutre) : sans ça, la création de l'index échoue.

CREATE UNIQUE INDEX IF NOT EXISTS cross_post_jobs_un_seul_publish_actif_par_article_plateforme
ON public.cross_post_jobs (inventaire_id, platform)
WHERE action = 'publish'
  AND status IN ('pending','processing','needs_user')
  AND inventaire_id IS NOT NULL;

COMMENT ON INDEX public.cross_post_jobs_un_seul_publish_actif_par_article_plateforme IS
'Un seul job de publication ACTIF par article et par plateforme (feu vert Nico, 23/09/2026). Bonnet Jack & Jones de lesnesarthur : deux jobs Vinted actifs pour le meme article, 18/09 et 19/09. Ne couvre QUE action=publish : republish et delete ont leurs propres machines a etats.';

-- ── LA RPC : 'needs_user' ENTRE DANS LA GARDE, ET LE REFUS NE COÛTE RIEN ────
-- Deux changements, et rien d'autre dans cette fonction :
--   · la liste de statuts de `v_conflicts` gagne 'needs_user' — c'est LE trou
--     par lequel le bonnet est passé ;
--   · l'INSERT est protégé : si l'index refuse malgré tout (course entre deux
--     clics, deux onglets), on REND LES PÉPITES et on répond proprement, au
--     lieu de laisser remonter un 23505 technique.
--     ⚠️ Le débit a lieu AVANT l'insert, et un RETURN depuis un bloc EXCEPTION
--     ne l'annule pas : on le défait explicitement (portefeuille + réservation).
--     Le coin_ledger, lui, n'est écrit qu'APRÈS l'insert — rien à défaire là.
...
BEGIN
  ...
  WHERE NULLIF(j->>'inventaire_id','') IS NOT NULL
    AND EXISTS (
      ...
      WHERE c.user_id = v_user
        ...
        AND (
          -- 'needs_user' AJOUTÉ le 2026-09-23 : un job qui attend une réponse
          -- de la personne est ACTIF. Sans lui, un second job partait sur le
          -- même article (bonnet Jack & Jones, lesnesarthur, 18 puis 19/09).
  ...
  -- ── L'INSERT, SOUS LA PROTECTION DE L'INDEX ──────────────────────────────
  -- Deux clics quasi simultanés passent tous deux la garde ci-dessus (elle lit
  -- avant d'écrire) ; l'index, lui, tranche. On rend alors les Pépites et on
  -- répond comme un refus ordinaire — jamais un 23505 à l'écran.
  BEGIN
