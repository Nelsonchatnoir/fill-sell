-- ═══════════════════════════════════════════════════════════════════════════
-- spend_coins_and_publish : refuse une plateforme EN PAUSE (platform_health)
-- 2026-09-09 — GO Nico (mécanisme de maintenance générique, par plateforme).
-- ═══════════════════════════════════════════════════════════════════════════
-- Ceinture derrière le grisage de l'app : un lot qui contient une plateforme
-- paused=true est refusé AVANT tout débit, avec la raison dédiée
-- « platform_paused », la liste des plateformes et les textes utilisateur
-- (message_fr / message_en) pour que l'app affiche la même phrase que le
-- bandeau. Aucune autre plateforme du lot n'est touchée : l'app retire la
-- plateforme en pause et republie le reste.
-- FAIL-SAFE (non négociable) : platform_health illisible → v_en_pause NULL →
-- la fonction continue exactement comme avant. Jamais une pause par accident.
-- Corps = copie INTÉGRALE de la définition en prod (20260906233000, marqueurs
-- v_ebay_api_seul + clause pending/processing vérifiés en base le 09/09),
-- plus le bloc « PLATEFORME EN PAUSE » et sa variable. Idempotente.
--
-- ⚠️ Appliquée en prod le 09/09/2026 (~16 h 15) par AUTO-PATCH de la définition
-- en base (DO : replace() sur les deux ancres, patchee = true vérifié), et non
-- en rejouant ce fichier : la ligne supabase_migrations.schema_migrations
-- 20260909161000 a été insérée à la main. Rejouer ce fichier est sans danger
-- (même corps), mais inutile.
...
BEGIN
  ...
  -- platform_health.paused = true : refus AVANT tout débit, raison dédiée
  -- « platform_paused » + plateformes + textes utilisateur. FAIL-SAFE : table
  -- illisible → NULL → on continue comme avant (jamais une pause par accident).
  BEGIN
