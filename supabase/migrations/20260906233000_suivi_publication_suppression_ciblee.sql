...
-- Tout ce qui compare created_at(delete) > created_at(publish) concluait
-- « retirée » : pastille eBay absente, garde already_published trompée,
-- trigger de voie exposé au même tri. La voie formulaire a le même trou dès
-- qu'un publish est relancé après un retrait.
--
-- ⚠️ Touche la garde already_published de spend_coins_and_publish (TOUTES
-- les plateformes) et le trigger de voie eBay. Corps = définitions PROD
-- relues par pg_get_functiondef le 06/09 + les seules touches ci-dessous.
--
-- 1. annonce_id_de_job(platform_listing_id, listing_url) : identifiant
--    d'annonce d'un job (colonne, sinon lu dans l'URL : /itm/<id>, /items/<id>,
--    ou dernier nombre ≥ 6 chiffres) — IMMUTABLE, même règle que
--    idAnnonceDe() côté app.
-- 2. spend_coins_and_publish : un delete ne « couvre » une publication que
--    s'il porte le MÊME identifiant d'annonce (quand les deux en ont un),
--    sinon s'il est postérieur à COALESCE(published_at, created_at).
-- 3. cross_post_jobs_voie_ebay : dernière publication = dernière MISE EN
--    LIGNE (COALESCE(published_at, created_at) DESC).
--
-- Vérification après application :
--   SELECT annonce_id_de_job(NULL, 'https://www.ebay.fr/itm/820094306007');            -- 820094306007
--   SELECT annonce_id_de_job('9880673349', 'https://www.vinted.fr/items/1-x');         -- 9880673349
--   SELECT prosrc LIKE '%annonce_id_de_job%' FROM pg_proc WHERE proname = 'spend_coins_and_publish';
--   SELECT prosrc LIKE '%COALESCE(j.published_at, j.created_at) DESC%' FROM pg_proc WHERE proname = 'cross_post_jobs_voie_ebay';
-- ═══════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.annonce_id_de_job(p_listing_id text, p_listing_url text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT COALESCE(
    NULLIF(btrim(p_listing_id), ''),
    substring(p_listing_url from '/itm/([0-9]+)'),
    substring(p_listing_url from '/items/([0-9]+)'),
    substring(p_listing_url from '/([0-9]{6,})(?:[/?#]|$)')
  );
$$;
          ...
          OR (
            ...
            AND NOT EXISTS (
              ...
              WHERE d.user_id = v_user
                ...
                -- 2026-09-06 : la suppression doit VISER cette publication —
                -- même identifiant d'annonce quand les deux le portent, sinon
                -- postérieure à la MISE EN LIGNE (published_at, pas created_at :
                -- un publish relancé après un retrait a une created_at plus
                -- ancienne que le delete de l'annonce précédente). Même règle
                -- que computeRemovalInfo (src/utils/publicationState.js).
                AND (
                  CASE
